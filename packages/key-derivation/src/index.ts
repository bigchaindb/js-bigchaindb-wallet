...
import { bufferToUint8Array, keyFactory } from './utils';
...
export class BigChainWallet {
  ...
  derive(derivationPath: string): Buffer {
  ...
  getKeypairFromDerivedKey(key: Buffer): KeyPair {
    const uInt8Key = bufferToUint8Array(key);
    const publicKey = (output?: KeyOutput) => keyFactory(getPublicKey(uInt8Key as Buffer, false), output);
    const privateKey = (output?: KeyOutput) => keyFactory(uInt8Key, output, 'secret');
    return { publicKey, privateKey };
  ...
  getDHKeyPair(account?: number): KeyPair {
    ...
    const publicKeyBuffer = ed2curve.convertPublicKey(keyPair.publicKey() as Uint8Array);
    const privateKeyBuffer = ed2curve.convertPrivateKey(keyPair.privateKey() as Uint8Array);
  ...
  getDHPublicKey(account?: number, output?: KeyOutput) {
    const publicKeyBuffer = ed2curve.convertPublicKey(this.getKeypair(account).publicKey() as Uint8Array);
    return keyFactory(publicKeyBuffer, output);
  }

  getDHPrivateKey(account?: number, output?: KeyOutput) {
    const privateKeyBuffer = ed2curve.convertPrivateKey(this.getFullPrivateKey(account) as Uint8Array);
